...
public class Delivery {
	private int currentIdCustomer=1;
    ...
    public int newCustomer(String name, String address, String phone, String email) throws DeliveryException {
        ...
        Customer c=new Customer(name,address,phone,email,currentIdCustomer);
        currentIdCustomer++;
    ...
    public List<String> listCustomers(){
        return customers.values().stream().sorted(Comparator.comparing(Customer::getNome)).map(Customer::toString).collect(Collectors.toList());
    }
    ...
    public void newMenuItem(String description, double price, String category, int prepTime){
        menues.add(new Menu(description,price,category,prepTime));
    }
    ...
    public List<String> findItem(String search){
    	...
    	if(s.equals(""))
    		return menues.stream()
    				.sorted(Comparator.comparing(Menu::getCategoria).thenComparing(Menu::getDescrizione))
            		.map(Menu::toString)
            		.collect(Collectors.toList());
    	return menues.stream()
        		.filter(m->m.getDescrizione().toLowerCase().contains(s))
        		.sorted(Comparator.comparing(Menu::getCategoria).thenComparing(Menu::getDescrizione))
        		.map(Menu::toString)
        		.collect(Collectors.toList());
    }
    
    public List<Menu> findMenu(String search) {
    	return menues.stream()
    			.filter(m->m.getDescrizione().toLowerCase().contains(search.toLowerCase()))
    			.collect(Collectors.toList());
    }
    ...
    public int newOrder(int customerId){
    	Order o =new Order(customerId,currentIdOrder);
    	currentIdOrder++;
    ...
    public int addItem(int orderId, String search, int qty) throws DeliveryException {
        if(!orders.containsKey(orderId) || findItem(search).size()!=1)
        	throw new DeliveryException();
        return orders.get(orderId).addItem(findMenu(search).get(0), qty);
    }
    ...
    public List<String> showOrder(int orderId) throws DeliveryException {
    	if(!orders.containsKey(orderId))
    		throw new DeliveryException();
        return orders.get(orderId).getItems();
    }
    ...
    public double totalOrder(int orderId) throws DeliveryException {
    	if(!orders.containsKey(orderId))
    		throw new DeliveryException();
    	return orders.get(orderId).getPrezzo();
    }
    ...
    public int confirm(int orderId) throws DeliveryException {
    	if(!orders.containsKey(orderId) || orders.get(orderId).getStatus()!=OrderStatus.NEW)
    		throw new DeliveryException();
    	return orders.get(orderId).confirm();
    }
    ...
    public int start(int orderId) throws DeliveryException {
    	if(!orders.containsKey(orderId) || orders.get(orderId).getStatus()!=OrderStatus.CONFIRMED)
    		throw new DeliveryException();
    	return orders.get(orderId).start();
    }
    ...
    public int deliver(int orderId) throws DeliveryException {
    	if(!orders.containsKey(orderId) || orders.get(orderId).getStatus()!=OrderStatus.PREPARATION)
    		throw new DeliveryException();
    	return orders.get(orderId).deliver();
    }
    ...
    public void complete(int orderId) throws DeliveryException {
    	if(!orders.containsKey(orderId) || orders.get(orderId).getStatus()!=OrderStatus.ON_DELIVERY)
    		throw new DeliveryException();
    ...
    public double totalCustomer(int customerId){
        return orders.values().stream()
        		...
        		.collect(Collectors.summingDouble(Order::getPrezzo));
    }
    ...
    public SortedMap<Double,List<String>> bestCustomers(){
        return customers.values().stream()
        		.collect(Collectors.groupingBy(c->totalCustomer(c.getId()),
        				() -> new TreeMap<>(),
        				Collectors.mapping(Customer::toString, Collectors.toList())));
    }
